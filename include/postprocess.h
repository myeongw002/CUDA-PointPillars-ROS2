...
struct Bndbox
{
  ...
  Bndbox() {}
  Bndbox(
    float x_, float y_, float z_, float w_, float l_, float h_, float rt_, int id_, float score_)
...
public:
  explicit PostProcessCuda(cudaStream_t stream = 0);
  ~PostProcessCuda();

  int doPostprocessCuda(
